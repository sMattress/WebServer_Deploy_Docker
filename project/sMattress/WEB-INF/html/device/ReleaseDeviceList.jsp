		...
		<div class="row clearfix" style="margin-top: 90px">
			...
			<div class="col-md-8 column">
				<div class="tabbable" id="tabs-device">
					...
					<div class="tab-content" style="margin: 30px 10px" S>
						<div class="tab-pane active" id="panel-releasedevicelist">
							<div class="table-responsive">
								<table class="table table-hover" style="font-size: 18px">
									...
									<tbody style="font-size: 15px">
										<c:forEach items="${releasedeviceslist}" var="releasedevices">
											<tr>
												<td>${releasedevices.IId}</td>
												<td>${releasedevices.vcName}</td>
												<td><c:if test="${releasedevices.IStatus==0}">
														<span class="badge" style="font-size: 15px;background-color: gray;">出厂</span>
													</c:if></td>
												<td>${releasedevices.TTime}</td>												
											</tr>
										</c:forEach>
									</tbody>
								...
								<ul class="pager">
									<c:choose>
										<c:when test="${page.pageNo>1}">
											<li><a
												href="<%=basePath%>deviceManager/releaseDeviceList?pageNo=${page.pageNo-1}">&larr;
													上一页</a>
											<li>
										</c:when>
										<c:otherwise>
									...
									<c:choose>
										<c:when test="${page.pageNo < page.totalPages}">
											<li><a
												href="<%=basePath%>deviceManager/releaseDeviceList?pageNo=${page.pageNo+1}">下一页
													&rarr;</a>
											<li>
										</c:when>
										<c:otherwise>
