			...
			<div class="col-md-8 column">
				<div class="tabbable" id="tabs-device">
					...
					<div class="tab-content" style="margin: 30px 10px">
						<div class="tab-pane active" id="panel-invalidatedevicelist">
							<div class="table-responsive">
								<table class="table table-hover" style="font-size: 18px">
									...
									<tbody style="font-size: 15px">
										<c:forEach items="${invalidatedeviceslist}"
											var="invalidatedevices">
											<tr>
												<td>${invalidatedevices.IId}</td>
												<td>${invalidatedevices.vcName}</td>
												<td><c:if test="${invalidatedevices.IStatus==2}">
														<span class="badge"
															style="background-color: red; font-size: 15px"">报废</span>
													</c:if>
												<td>${invalidatedevices.TTime}</td>
												<td><button type="button" class="btn btn-warning round"
														onclick="location.href='<%=basePath%>deviceManager/changevalidatedevice?IId=${invalidatedevices.IId}&IStatus=${invalidatedevices.IStatus}'">工作</button></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
								<ul class="pager">
									<c:choose>
										<c:when test="${page.pageNo>1}">
											<li><a
												href="<%=basePath%>deviceManager/invalidatedeviceList?pageNo=${page.pageNo-1}">&larr;
													上一页</a>
											<li>
										</c:when>
										<c:otherwise>
									...
									<c:choose>
										<c:when test="${page.pageNo < page.totalPages}">
											<li><a
												href="<%=basePath%>deviceManager/invalidatedeviceList?pageNo=${page.pageNo+1}">下一页
													&rarr;</a>
											<li>
										</c:when>
										<c:otherwise>
